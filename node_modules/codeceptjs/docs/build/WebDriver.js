...
@param pathToFile local file path relative to codecept.json config file.
  ...
  async attachFile(locator, pathToFile) {
    ...
    if (this.options.remoteFileUpload) {
      ...
      // once webdriverio API provides a way to send arbiterary requets, replace this :)
      try {
        this.debugSection('File', 'Uploading file to remote server');
        const uploadEndpoint = `http://${this.options.host}:${this.options.port}${this.options.path}/session/${this.browser.sessionId}/file`;
        const restClient = new REST();
        const response = await restClient.sendPostRequest(
          uploadEndpoint,
          { file: fileCompressed },
        );
        if (!response.data || !response.data.value) throw new Error(`Invalid request to ${uploadEndpoint}`);
        file = response.data.value;
      } catch (err) {
...
@param locator element located by CSS|XPath|strict locator.
  ...
  async grabHTMLFrom(locator) {
    const html = await this.browser.getHTML(locator.type, locator.value);
...
```
  ...
  async grabCurrentUrl() {
    const res = await this.browser.getCurrentUrl();
...
@param sec (optional) time in seconds to wait.
  ...
  async waitInUrl(urlPart, sec = null) {
    ...
    return client
      .waitUntil(function () {
        return this.getCurrentUrl().then((res) => {
